...
function loadTemplate(templateName) {
  ...
  if (templates[templateName]) {
    templates[templateName].items.forEach(item => {
      ...
      // Setze PrÃ¼fvermerk (optional - kann leer bleiben oder Standard-Text)
      const pruefInput = document.getElementById(`pruef-${rowNum}`);
      if (pruefInput) {
        pruefInput.value = 'âœ“'; // Einfaches HÃ¤kchen als BestÃ¤tigung
      }
    });
  }
}
...
async function submitForm(event) {
  ...
  if (!firmaChecked) {
    isValid = false;
    messageDiv.innerHTML = '<div class="form-message error">Bitte wÃ¤hlen Sie mindestens eine Firma aus.</div>';
    return;
  }
  
  if (!isValid) {
    messageDiv.innerHTML = '<div class="form-message error">Bitte fÃ¼llen Sie alle Pflichtfelder aus.</div>';
    return;
  }
  ...
  try {
    ...
    if (response.ok) {
      messageDiv.innerHTML = '<div class="form-message success">âœ“ GefÃ¤hrdungsbeurteilung erfolgreich gesendet!</div>';
      
      // Formular nach 2 Sekunden zurÃ¼cksetzen
      setTimeout(() => {
...
function collectFormData() {
  // Basis-Daten
  const data = {
    formularTyp: 'BetriebsGefaehrdungsbeurteilung', // NEU: Typ fÃ¼r n8n Switch
